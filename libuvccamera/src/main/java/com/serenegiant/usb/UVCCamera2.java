...
public class UVCCamera2 {
	private static final boolean DEBUG = false;	// TODO set false when releasing
	private static final String TAG = UVCCamera2.class.getSimpleName();
...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ(不要在这里清除，因为会调用 nativeDestroy)
	...
	public static List<Size> getSupportedSize(final int type, final String supportedSize) {
		...
		try {
			...
			Log.d(TAG, "getSupportedSize: " + json.toString());
...
//================================================================================
	...
	private final String getUSBFSName(final UsbControlBlock ctrlBlock) {
		...
		Log.d(TAG, "getUSBFSName: original USB device name: " + name);

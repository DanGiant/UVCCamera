				...
				DL_FOREACH(stream_if->format_descs, fmt_desc)
				{
					writer.StartObject();
					{
						...
						case UVC_VS_FORMAT_MJPEG:
                            ...
                            DL_FOREACH(fmt_desc->frame_descs, frame_desc)
                            {
                                if (frame_desc->intervals && frame_desc->intervals[0] != 0) {
                                    for(int i=0; frame_desc->intervals[i] != 0; i++) {
                                        uint32_t frame_rate = 10000000 / frame_desc->intervals[i];
                                        snprintf(buf, sizeof(buf), "%dx%d@%d",
                                                 frame_desc->wWidth, frame_desc->wHeight, frame_rate);
                                        buf[sizeof(buf)-1] = '\0';
                                        writer.String(buf);
                                    }
                                }
                                else {
                                    uint32_t frame_rate = 10000000 / frame_desc->dwDefaultFrameInterval;

...
void UVCCamera2::clearCameraParams() {
...
int UVCCamera2::setStatusCallback(JNIEnv *env, jobject status_callback_obj) {
	ENTER();
...
int UVCCamera2::setButtonCallback(JNIEnv *env, jobject button_callback_obj) {
	ENTER();
	int result = EXIT_FAILURE;
	if (mButtonCallback) {
...
char *UVCCamera2::getSupportedSize() {
	ENTER();
	if (mDeviceHandle) {
		UVCDiags params;
...
int UVCCamera2::setPreviewSize(int width, int height, int min_fps, int max_fps, int mode, float bandwidth) {
	ENTER();
	int result = EXIT_FAILURE;
	if (mPreview) {
...
int UVCCamera2::setPreviewDisplay(ANativeWindow *preview_window) {
	ENTER();
	int result = EXIT_FAILURE;
	if (mPreview) {
...
int UVCCamera2::setFrameCallback(JNIEnv *env, jobject frame_callback_obj, int pixel_format) {
	ENTER();
	int result = EXIT_FAILURE;
	if (mPreview) {
...
int UVCCamera2::getCtrlSupports(uint64_t *supports) {
	ENTER();
...
int UVCCamera2::getProcSupports(uint64_t *supports) {
	ENTER();
	uvc_error_t ret = UVC_ERROR_NOT_FOUND;
	if (LIKELY(mDeviceHandle)) {
		if (!mPUSupports) {
 ...
 * カメラコントロール設定の下請け
 */
int UVCCamera2::internalSetCtrlValue(control_value_t &values, int8_t value,
...
int UVCCamera2::internalSetCtrlValue(control_value_t &values, uint8_t value,
...
int UVCCamera2::internalSetCtrlValue(control_value_t &values, uint8_t value1, uint8_t value2,
...
int UVCCamera2::internalSetCtrlValue(control_value_t &values, int8_t value1, uint8_t value2,
...
int UVCCamera2::internalSetCtrlValue(control_value_t &values, int8_t value1, uint8_t value2, uint8_t value3,
 ...
 * カメラコントロール設定の下請け
 */
int UVCCamera2::internalSetCtrlValue(control_value_t &values, int16_t value,
 ...
 * カメラコントロール設定の下請け
 */
int UVCCamera2::internalSetCtrlValue(control_value_t &values, uint16_t value,
 ...
 * カメラコントロール設定の下請け
 */
int UVCCamera2::internalSetCtrlValue(control_value_t &values, int32_t value,
 ...
 * カメラコントロール設定の下請け
 */
int UVCCamera2::internalSetCtrlValue(control_value_t &values, uint32_t value,
...
//======================================================================
// スキャニングモード
int UVCCamera2::updateScanningModeLimit(int &min, int &max, int &def) {
	ENTER();
...
// スキャニングモードをセット
int UVCCamera2::setScanningMode(int mode) {
	ENTER();
...
// スキャニングモード設定を取得
int UVCCamera2::getScanningMode() {
...
//======================================================================
// 露出モード
int UVCCamera2::updateExposureModeLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// 露出をセット
int UVCCamera2::setExposureMode(int mode) {
	ENTER();
	int r = UVC_ERROR_ACCESS;
...
// 露出設定を取得
int UVCCamera2::getExposureMode() {
...
//======================================================================
// 露出優先設定
int UVCCamera2::updateExposurePriorityLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// 露出優先設定をセット
int UVCCamera2::setExposurePriority(int priority) {
	ENTER();
	int r = UVC_ERROR_ACCESS;
...
// 露出優先設定を取得
int UVCCamera2::getExposurePriority() {
...
//======================================================================
// 露出(絶対値)設定
int UVCCamera2::updateExposureLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// 露出(絶対値)設定をセット
int UVCCamera2::setExposure(int ae_abs) {
	ENTER();
	int r = UVC_ERROR_ACCESS;
...
//======================================================================
// 露出(相対値)設定
int UVCCamera2::updateExposureRelLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// 露出(相対値)設定をセット
int UVCCamera2::setExposureRel(int ae_rel) {
	ENTER();
	int r = UVC_ERROR_ACCESS;
...
//======================================================================
// オートフォーカス
int UVCCamera2::updateAutoFocusLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// オートフォーカスをon/off
int UVCCamera2::setAutoFocus(bool autoFocus) {
	ENTER();
...
//======================================================================
// フォーカス(絶対値)調整
int UVCCamera2::updateFocusLimit(int &min, int &max, int &def) {
	ENTER();
...
// フォーカス(絶対値)を設定
int UVCCamera2::setFocus(int focus) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
	if (mCtrlSupports & CTRL_FOCUS_ABS) {
...
//======================================================================
// フォーカス(相対値)調整
int UVCCamera2::updateFocusRelLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
...
// フォーカス(相対値)を設定
int UVCCamera2::setFocusRel(int focus_rel) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
	if (mCtrlSupports & CTRL_FOCUS_REL) {
...
/*
// フォーカス(シンプル)調整
int UVCCamera2::updateFocusSimpleLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
...
// フォーカス(シンプル)を設定
int UVCCamera2::setFocusSimple(int focus) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
	if (mCtrlSupports & CTRL_FOCUS_SIMPLE) {
...
//======================================================================
// 絞り(絶対値)調整
int UVCCamera2::updateIrisLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
...
// 絞り(絶対値)を設定
int UVCCamera2::setIris(int iris) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
	if (mCtrlSupports & CTRL_IRIS_ABS) {
...
//======================================================================
// 絞り(相対値)調整
int UVCCamera2::updateIrisRelLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
...
// 絞り(相対値)を設定
int UVCCamera2::setIrisRel(int iris_rel) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
	if (mCtrlSupports & CTRL_IRIS_REL) {
...
//======================================================================
// Pan(絶対値)調整
int UVCCamera2::updatePanLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
...
//======================================================================
// Tilt(絶対値)調整
int UVCCamera2::updateTiltLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
	if (mCtrlSupports & CTRL_PANTILT_ABS) {
		update_ctrl_values(mDeviceHandle, mPan, mTilt, uvc_get_pantilt_abs);
...
// Tilt(絶対値)を設定
int UVCCamera2::setTilt(int tilt) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
	if (mCtrlSupports & CTRL_PANTILT_ABS) {
		ret = update_ctrl_values(mDeviceHandle, mPan, mTilt, uvc_get_pantilt_abs);
...
//======================================================================
// Roll(絶対値)調整
int UVCCamera2::updateRollLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
...
// Roll(絶対値)を設定
int UVCCamera2::setRoll(int roll) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
	if (mCtrlSupports & CTRL_ROLL_ABS) {
...
//======================================================================
int UVCCamera2::updatePanRelLimit(int &min, int &max, int &def) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}

int UVCCamera2::setPanRel(int pan_rel) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}

int UVCCamera2::getPanRel() {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}
...
//======================================================================
int UVCCamera2::updateTiltRelLimit(int &min, int &max, int &def) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}

int UVCCamera2::setTiltRel(int tilt_rel) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}

int UVCCamera2::getTiltRel() {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}
...
//======================================================================
int UVCCamera2::updateRollRelLimit(int &min, int &max, int &def) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}

int UVCCamera2::setRollRel(int roll_rel) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}

int UVCCamera2::getRollRel() {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}
...
//======================================================================
// プライバシーモード
int UVCCamera2::updatePrivacyLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
...
// プライバシーモードを設定
int UVCCamera2::setPrivacy(int privacy) {
	ENTER();
	int ret = UVC_ERROR_ACCESS;
	if (mCtrlSupports & CTRL_PRIVACY) {
...
/*
// DigitalWindow
int UVCCamera2::updateDigitalWindowLimit(...not defined...) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}
...
// DigitalWindowを設定
int UVCCamera2::setDigitalWindow(int top, int reft, int bottom, int right) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}
...
int UVCCamera2::getDigitalWindow(int &top, int &reft, int &bottom, int &right) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}
...
/*
// DigitalRoi
int UVCCamera2::updateDigitalRoiLimit(...not defined...) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}
...
// DigitalRoiを設定
int UVCCamera2::setDigitalRoi(int top, int reft, int bottom, int right) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}
...
// DigitalRoiの現在値を取得
int UVCCamera2::getDigitalRoi(int &top, int &reft, int &bottom, int &right) {
	ENTER();
	// FIXME not implemented yet
	RETURN(UVC_ERROR_ACCESS, int);
}
...
int UVCCamera2::updateBacklightCompLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
int UVCCamera2::setBacklightComp(int backlight) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_BACKLIGHT) {
...
int UVCCamera2::getBacklightComp() {
	ENTER();
	if (mPUSupports & PU_BACKLIGHT) {
...
//======================================================================
// 明るさ
int UVCCamera2::updateBrightnessLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
int UVCCamera2::setBrightness(int brightness) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_BRIGHTNESS) {
...
//======================================================================
// コントラスト調整
int UVCCamera2::updateContrastLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// コントラストを設定
int UVCCamera2::setContrast(uint16_t contrast) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_CONTRAST) {
...
//======================================================================
// オートコントラスト
int UVCCamera2::updateAutoContrastLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// オートコントラストをon/off
int UVCCamera2::setAutoContrast(bool autoContrast) {
	ENTER();
...
bool UVCCamera2::getAutoContrast() {
	ENTER();
	int r = UVC_ERROR_ACCESS;
	if LIKELY((mDeviceHandle) && (mPUSupports & PU_CONTRAST_AUTO)) {
...
//======================================================================
// シャープネス調整
int UVCCamera2::updateSharpnessLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// シャープネスを設定
int UVCCamera2::setSharpness(int sharpness) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_SHARPNESS) {
...
//======================================================================
// ゲイン調整
int UVCCamera2::updateGainLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// ゲインを設定
int UVCCamera2::setGain(int gain) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_GAIN) {
//		LOGI("gain:%d", gain);
...
//======================================================================
// オートホワイトバランス(temp)
int UVCCamera2::updateAutoWhiteBlanceLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
int UVCCamera2::setAutoWhiteBlance(bool autoWhiteBlance) {
	ENTER();
	int r = UVC_ERROR_ACCESS;
...
bool UVCCamera2::getAutoWhiteBlance() {
	ENTER();
	int r = UVC_ERROR_ACCESS;
	if LIKELY((mDeviceHandle) && (mPUSupports & PU_WB_TEMP_AUTO)) {
...
//======================================================================
// オートホワイトバランス(compo)
int UVCCamera2::updateAutoWhiteBlanceCompoLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
int UVCCamera2::setAutoWhiteBlanceCompo(bool autoWhiteBlanceCompo) {
	ENTER();
	int r = UVC_ERROR_ACCESS;
...
bool UVCCamera2::getAutoWhiteBlanceCompo() {
	ENTER();
	int r = UVC_ERROR_ACCESS;
	if LIKELY((mDeviceHandle) && (mPUSupports & PU_WB_COMPO_AUTO)) {
...
//======================================================================
// ホワイトバランス色温度調整
int UVCCamera2::updateWhiteBlanceLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// ホワイトバランス色温度を設定
int UVCCamera2::setWhiteBlance(int white_blance) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_WB_TEMP) {
...
//======================================================================
// ホワイトバランスcompo調整
int UVCCamera2::updateWhiteBlanceCompoLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// ホワイトバランスcompoを設定
int UVCCamera2::setWhiteBlanceCompo(int white_blance_compo) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_WB_COMPO) {
...
int UVCCamera2::getWhiteBlanceCompo() {
	ENTER();
	if (mPUSupports & PU_WB_COMPO) {
...
//======================================================================
// ガンマ調整
int UVCCamera2::updateGammaLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// ガンマを設定
int UVCCamera2::setGamma(int gamma) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_GAMMA) {
//		LOGI("gamma:%d", gamma);
...
//======================================================================
// 彩度調整
int UVCCamera2::updateSaturationLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// 彩度を設定
int UVCCamera2::setSaturation(int saturation) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_SATURATION) {
...
//======================================================================
// 色相調整
int UVCCamera2::updateHueLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
//======================================================================
// オート色相
int UVCCamera2::updateAutoHueLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// オート色相をon/off
int UVCCamera2::setAutoHue(bool autoHue) {
	ENTER();
...
//======================================================================
// 電源周波数によるチラつき補正
int UVCCamera2::updatePowerlineFrequencyLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// 電源周波数によるチラつき補正を設定
int UVCCamera2::setPowerlineFrequency(int frequency) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// 電源周波数によるチラつき補正値を取得
int UVCCamera2::getPowerlineFrequency() {
	ENTER();
	if (mPUSupports & PU_POWER_LF) {
		uint8_t value;
...
//======================================================================
// ズーム(abs)調整
int UVCCamera2::updateZoomLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// ズーム(abs)を設定
int UVCCamera2::setZoom(int zoom) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mCtrlSupports & CTRL_ZOOM_ABS) {
...
//======================================================================
// ズーム(相対値)調整
int UVCCamera2::updateZoomRelLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
// ズーム(相対値)を設定
int UVCCamera2::setZoomRel(int zoom) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mCtrlSupports & CTRL_ZOOM_REL) {
...
int UVCCamera2::updateDigitalMultiplierLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
int UVCCamera2::setDigitalMultiplier(int multiplier) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_DIGITAL_MULT) {
...
int UVCCamera2::getDigitalMultiplier() {
	ENTER();
	if (mPUSupports & PU_DIGITAL_MULT) {
...
int UVCCamera2::updateDigitalMultiplierLimitLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
int UVCCamera2::setDigitalMultiplierLimit(int multiplier_limit) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_DIGITAL_LIMIT) {
...
int UVCCamera2::getDigitalMultiplierLimit() {
	ENTER();
	if (mPUSupports & PU_DIGITAL_LIMIT) {
...
//======================================================================
// AnalogVideoStandard
int UVCCamera2::updateAnalogVideoStandardLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
int UVCCamera2::setAnalogVideoStandard(int standard) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_AVIDEO_STD) {
...
int UVCCamera2::getAnalogVideoStandard() {
	ENTER();
	if (mPUSupports & PU_AVIDEO_STD) {
...
int UVCCamera2::updateAnalogVideoLockStateLimit(int &min, int &max, int &def) {
	ENTER();
	int ret = UVC_ERROR_IO;
...
int UVCCamera2::setAnalogVideoLockState(int state) {
	ENTER();
	int ret = UVC_ERROR_IO;
	if (mPUSupports & PU_AVIDEO_LOCK) {
...
int UVCCamera2::getAnalogVideoLockState() {
	ENTER();
	if (mPUSupports & PU_AVIDEO_LOCK) {
